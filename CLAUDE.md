...
- **Task types**: `task_types` table with color-coded badges. Settings CRUD in `components/settings/task-type-manager.tsx`. "My tasks" view groups by type with colored dot headers; other filters show flat table with type column. Type badges also show on project task rows and on the My Tasks / Personal Todos dashboard cards.
- **Task ordering (drag-reorder)**: `tasks.sort_order` int column (migration 016). The Tasks page lists (My tasks / All / Personal, not Completed) are drag-sortable via `@dnd-kit/sortable` in `tasks-client.tsx`. Each list/group is wrapped in its own `Reorder` (one isolated `DndContext` per list, so the grouped "My tasks" view reorders within a type group only). Queries order by `sort_order` then `created_at desc` so new tasks (default 0) surface at the top. `handleDragEnd` renumbers the visible list 0..n and persists only changed rows.
- **Task checklists (subtasks)**: `task_checklist_items` table (migration 016, RLS mirrors parent-task visibility/editability). Managed in `TaskDetailPanel` (add via input/Enter, toggle, delete). List rows show a `done/total` progress badge (`ChecklistBadge`); the panel reports changes up via `onChecklistChange` so badges stay in sync without a refetch. Task selects include `task_checklist_items(id, completed_at)`.
- **Task templates**: `task_templates` table (migration 016, `items text[]`; authenticated-all RLS like task_types). Settings CRUD in `components/settings/task-template-manager.tsx` at `/settings/task-templates` (items edited one-per-line). "Apply template…" dropdown in `TaskDetailPanel` appends the template's items to the current checklist.
