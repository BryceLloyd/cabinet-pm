...
- **Testing RLS** (no local Postgres): impersonate a role in a rolled-back transaction — `begin; set local role authenticated; select set_config('request.jwt.claims', '{"sub":"<user-uuid>","role":"authenticated"}', true); <write attempts>; rollback;`. RLS UPDATE denied = 0 rows affected (no error); WITH CHECK / INSERT denied = "violates row-level security policy" error. Wrap each attempt in a plpgsql block that catches the error.
- **Shared dev/prod DB — sequence destructive migrations**: dev and prod point at the same DB, so a migration that DROPs a column / tightens a constraint the *currently-deployed* code still uses breaks prod the moment it's pushed. Apply safe/additive parts anytime; apply the destructive part only AFTER the new code is merged + deployed. RLS lock-down also depends on data migrations (e.g. role rename) having run first.
- Auth uses middleware.ts to refresh JWT via `getUser()` on every request

## Patterns & Gotchas
- **Claude Preview login/forms**: `preview_fill` sets the DOM value but doesn't reliably update React controlled-input state, so form submits with empty values. Set the value via the native setter then dispatch `input`: `const s=Object.getOwnPropertyDescriptor(HTMLInputElement.prototype,'value').set; s.call(el,v); el.dispatchEvent(new Event('input',{bubbles:true}))`.
- **Stacked PRs + squash merge**: squash-merging a base branch auto-closes its stacked child PR and leaves the child conflicting (its commits aren't in the squashed master). Rebuild the child by `git checkout -b new origin/master` + `git cherry-pick <child-sha>`, then open a fresh PR — don't merge the old branch in.
- **`gh pr merge --delete-branch` in a worktree** prints `fatal: 'master' is already used by worktree at ...` (it can't check out master locally to clean up) but the merge itself **succeeds** — verify with `gh pr view <n> --json state`; the error is harmless.
...
## Permissions & roles
- **Four roles, role decides everything**: `admin` (all) / `office` (Office + Production incl. production settings; not team/system settings) / `factory` (production floor: cut-edge/painting/assembly/hardware sections) / `site` (installation only). The old `profiles.office_access`/`production_access` columns and the `member` role were removed (migration 027).
- **App-layer source of truth**: `lib/production/access.ts` — `canSeeOffice` (admin/office), `canSeeProduction` (all), `canSeeProductionSettings` (admin/office), `canSeeStage`. `middleware.ts` derives Office/Production view access from role and guards admin-only settings + `/production/cutlists*` (office/admin only). Nav components (`header-nav`, `bottom-tab-bar`) take a `showCutlists` prop; per-page create/edit controls take a `canManage`/`canPlace` bool from the page's role fetch.
- **DB lock-down (RLS, migrations 028–030)**: SECURITY DEFINER helpers `auth_role()`, `is_admin()`, `is_office()` (admin OR office) back per-table policies. Reads stay open to authenticated; writes: office-domain (projects, rooms, tasks, calendar, cutlists, material_orders, production_items) = office/admin; system settings (phases, task_types, event_types, task_templates, business_info) = admin; production settings (materials, hardware_catalog, paint_types, suppliers, production_stages/steps) = office/admin; `production_item_steps` = any role (sign-off, UPDATE only); `hardware_order_items` = office/admin place, **factory may set `status='received'` only** (a `WITH CHECK` policy — no RPC). `business_info` keeps a public SELECT (login page reads it pre-auth).
- **User creation**: admins create accounts directly via `POST /api/users/create` (service-role, admin-checked, sets `email_confirm:true`). Self-signup, the `allowed_emails` flow, and the login Sign-up/Magic-link tabs were removed — Supabase's built-in confirmation email was unreliable.
- **Cutlists** are an office/admin planning tool — hidden from factory/site (nav + middleware redirect).

## Production area
- **Office vs Production views**: view access is derived from role (see Permissions & roles), enforced in `middleware.ts`. The `Office | Production` toggle (`components/view-switch.tsx`, shown only when the user has both) swaps the header nav (`components/header-nav.tsx`, view-aware by pathname). Production nav lives in the main header (no sub-rail); set a person's role in Settings → Team.
