...
function SortableTaskTypeRow({
  ...
  return (
    ...
    >
          ...
          >
            <div className="grid grid-cols-4 gap-2">
              {PRESET_COLORS.map((c) => (
                <button
                  key={c}
                  onClick={() => onColorChange(taskType.id, c)}
      ...
      <input
        value={taskType.name}
        onChange={(e) => onNameChange(taskType.id, e.target.value)}
        onBlur={() => onNameBlur(taskType.id)}
        onKeyDown={(e) => {
      ...
      <button
        onClick={() => onArchive(taskType.id)}
        className="shrink-0 text-muted-foreground hover:text-foreground"
        title="Archive task type"
      >
        <Archive size={16} />
      </button>
    </li>
...
export function TaskTypeManager({ initialTaskTypes, isAdmin }: Props) {
  const router = useRouter();
  const supabase = createClient();
  const [editing, setEditing] = useState(false);
  const [types, setTypes] = useState<TaskType[]>(
    initialTaskTypes.filter((t) => !t.archived_at)
  );
  const [archivedTypes, setArchivedTypes] = useState<TaskType[]>(
    initialTaskTypes.filter((t) => !!t.archived_at)
  );
  ...
  const handleNameBlur = useCallback(
    async (id: string) => {
      ...
      if (!tt || !tt.name.trim()) return;
      await supabase
        .from("task_types")
        .update({ name: tt.name.trim() })
        .eq("id", id);
    },
    [types, supabase]
  );
  ...
  const handleColorChange = useCallback(
    async (id: string, color: string) => {
      ...
      await supabase.from("task_types").update({ color }).eq("id", id);
    },
    [supabase]
  );
  ...
  async function handleDragEnd(event: DragEndEvent) {
    ...
    await Promise.all(
      reordered.map((t, i) =>
        supabase.from("task_types").update({ sort_order: i }).eq("id", t.id)
      )
    );
  }
  ...
  async function handleAdd() {
    ...
    if (!error && data) {
      setTypes([...types, data as TaskType]);
    }
  }
    ...
    async (id: string) => {
      ...
      if (count && count > 0) {
        const proceed = confirm(
          `${count} task${count !== 1 ? "s use" : " uses"} the "${tt.name}" type. Archiving won't remove them, but this type won't be available for new tasks. Archive anyway?`
        );
        if (!proceed) return;
      }
      ...
      if (!error) {
        const archived = { ...tt, archived_at: new Date().toISOString() };
  ...
  async function handleRestore(id: string) {
    const tt = archivedTypes.find((t) => t.id === id);
    if (!tt) return;
    ...
    if (!error) {
      const restored = { ...tt, archived_at: null, sort_order: maxOrder + 1 };
      setArchivedTypes((prev) => prev.filter((t) => t.id !== id));
      setTypes((prev) => [...prev, restored as TaskType]);
    }
  }
  ...
  return (
    <section className="rounded-lg border bg-card">
      <div className="px-5 py-3.5 border-b flex items-center justify-between">
        <h2 className="font-medium">{editing ? "Edit task types" : "Task types"}</h2>
        {isAdmin && (
          <button
      ...
      {types.length === 0 && !editing && (
        <div className="px-5 py-8 text-center text-sm text-muted-foreground">
          No task types yet. {isAdmin && "Click \"Edit types\" to add some."}
        </div>
      )}
